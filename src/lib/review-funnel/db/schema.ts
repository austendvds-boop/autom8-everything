...
export const rfTenants = pgTable(
  "rf_tenants",
  {
    ...
    promoMessage: text("promo_message").default(
    ...
    smsLimitMonthly: integer("sms_limit_monthly").notNull().default(200),
    calendarLimit: integer("calendar_limit").notNull().default(999999),
    overageBillingEnabled: boolean("overage_billing_enabled").notNull().default(false),
    followUpNudgeEnabled: boolean("follow_up_nudge_enabled").notNull().default(false),
...
export const rfReviewRequests = pgTable(
  "rf_review_requests",
  {
    ...
    smsReplyRating: smallint("sms_reply_rating"),
    smsReplyReceivedAt: timestamp("sms_reply_received_at", { withTimezone: true }),
    ...
    gmbLinkSent: boolean("gmb_link_sent").notNull().default(false),
  ...
  (table) => ({
    ...
    smsReplyRatingValid: check(
      "rf_review_requests_sms_reply_rating_chk",
      sql`${table.smsReplyRating} BETWEEN 1 AND 5 OR ${table.smsReplyRating} IS NULL`,
    ),
  }),
)
